...
import { createTest } from 'src/api';
      ...
      onCreateSubmit: async () => {
        ...
        try {
          const result = await createTest({
            ...
            pages: pageItems.value.map((page) => page.elements.map((element) => ({
              elementType: element.idElement.type,
              questionSetShortId: element.idElement.questionSetShortId,
              questionShortId: element.idElement.questionShortId,
              variants: element.variants.map((variant) => variant.shortId),
            }))),
          });
          await router.push(`/teacher/tests/${result.shortId}`);
        } catch (error) {
          console.error(error);
          quasar.notify({
            type: 'negative',
